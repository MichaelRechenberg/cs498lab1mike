...
class ObjectRecognition:
    ...
    def __init__(self, root_model_dir, model_name):
        ...
        self.tf_graph = _load_tensorflow_graph(root_model_dir, model_name)
        ...
        # input image tensor
        self.image_tensor = lab_graph.get_tensor_by_name("image_tensor:0")
        ...
        self.boxes = lab_graph.get_tensor_by_name("detection_boxes:0")
        # classification scores for each detection (from 0 to 1)
        self.scores = lab_graph.get_tensor_by_name("detection_scores:0")
        # integer ids of the detected classes w.r.t COCO class labels
        self.classes = lab_graph.get_tensor_by_name("detection_classes:0")
        # number of non-zero score detections
        self.num_detections = lab_graph.get_tensor_by_name("num_detections:0")
    ...
    def _load_coco_labels(self):
        ...
        coco_label_map = {int(k): for (k,v) in coco_label_map.items()}
        return coco_label_map

    def _draw_bounding_box(image_arr, bounds):
